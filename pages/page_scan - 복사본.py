...
def _fast_ocr(img, lang="kor", psm=7, whitelist=""):
    """
    빠른 OCR(짧은 영역/ROI/보강용)
    """
    if pytesseract is None or img is None:
        return ""
    cfg = f"--oem 3 --psm {psm}"
...
def parse_arc(img, fast: bool = False):
    ...
    if img is None:
        return out
    img_orig = img  # ✅ fast 리사이즈 전 원본 보관(보강용)
    ...
    if not name_ko:
        # ✅ 상단이 정렬돼도, 스티커/손글씨는 ROI가 더 잘 잡히는 경우가 많음
        name_ko = _extract_name_from_roi(img, t_top)
    ...
    best_text, best_sc = "", -1
    for deg in (0, 90, 270):
        im = bot if deg == 0 else bot.rotate(deg, expand=True)
        ...
        # ✅ FAST면 0도에서 한글이 충분히 나오면 더 안 돌림(속도↑)
        if fast and deg == 0 and sc >= 25:
            break
